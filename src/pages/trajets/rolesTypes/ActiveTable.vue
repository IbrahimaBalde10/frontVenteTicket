...
<template>
  <VRow>
    <VCol cols="12">
      <VCard title="Utilisateurs actifs">
        <VCardText>
          <VTable>
            <thead>
              <tr>
            ...
            <tbody>
              <tr v-for="user in allUsers" :key="user.id">
                ...
                <td>{{ user.status }}</td>
                <td>
                  <VIcon @click="desactiveUser(user.id)" class="ms-2" color="warning" small>mdi-lock</VIcon>

                  <!-- <VIcon @click="desactiveUser(user.id)" class="ms-2" color="warning" small>mdi-pencil</VIcon> -->
