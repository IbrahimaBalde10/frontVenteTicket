...
<template>
  <VRow>
    <VCol cols="12">
      <VCard title="Utilisateurs inactifs">
        <VCardText>
          <VTable>
            <thead>
              <tr>
            ...
            <tbody>
              <tr v-for="user in allUsers" :key="user.id">
                ...
                <td>{{ user.status }}</td>
                <td>
                  <VIcon @click="activeUser(user.id)" class="ms-2" color="success" small>mdi-lock-open</VIcon>

                  <!-- <VIcon @click="activeUser(user.id)" class="ms-2" color="warning" small>mdi-pencil</VIcon> -->
